...
const API_ADDRESS: &str = "https://endbasic-service.azurewebsites.net";
...
impl Service for CloudService {
    ...
    async fn login(
        ...
        // TODO(jmmv): Given that the server can take a very long time to start up the first time,
        // we should watch out for a very long reply and tell the user what's going on.
        let response = self
            .client
    ...
    fn test_login() {
        ...
        async fn run(context: &mut TestContext) {
            ...
            match service.login(&access_token, &request).await.unwrap() {
                ...
                Err(response) => {
                    assert!(!response.missing_data.is_some());
                }
            }
        }
